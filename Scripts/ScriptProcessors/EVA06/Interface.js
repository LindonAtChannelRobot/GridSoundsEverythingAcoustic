...
{
	//
	local pos = theVolumeVoices.indexOf(component);
	...
	theSimpleGains[pos].setAttribute(theSimpleGains[pos].Gain, value);
...
// these clear out teh odd display elements in tables...
...
const var VelocityCurveOnOff = Content.getComponent("VelocityCurveOnOff");

const var ArpRetrigger = Synth.getMidiProcessor("ArpRetrigger");
const var arpLatchReset = Content.getComponent("arpLatchReset");

const var arpRanger = Synth.getMidiProcessor("arpRanger");
...
inline function onArpBypassControl(component, value)
{
    ...
    if (!value)
    {
        ArpRetrigger.setBypassed(true);
    }else{
        ArpRetrigger.setBypassed(1 - arpLatchReset.getValue());
        VelocityCurveOnOff.setValue(0);
        VelocityCurveOnOff.changed();
    }
    ...
    if (value && (Content.getComponent("IgnoreInput").getValue() == 1))
    {
        for (s in TheSamplers)
        {
            s.setAttribute(s.SamplerRepeatMode,2);
        }
    }else{
        for (s in TheSamplers)
        {
            s.setAttribute(s.SamplerRepeatMode,3);
        }
    }
...
inline function onArpChordsControl(component, value)
{
	//
	if(value)
	    Arpeggiator1.setAttribute(Arpeggiator1.SequenceComboBox, 6);
};

Content.getComponent("ArpChords").setControlCallback(onArpChordsControl);


inline function onNoteTieControl(component, value)
{
	//
	Arpeggiator1.setAttribute(11, value);
};

Content.getComponent("NoteTie").setControlCallback(onNoteTieControl);
...
inline function onIgnoreInputControl(component, value)
{
	...
	if (arpHold.getValue() == 1)
	    IgnoreInput.setValue(0);
	Arpeggiator1.setAttribute(17,value + 2);
    if (value && (Content.getComponent("ArpBypass").getValue() == 1))
    {
        for (s in TheSamplers)
        {
            s.setAttribute(s.SamplerRepeatMode,2);
...
{
	//
    Arpeggiator1.setAttribute(Arpeggiator1.Hold, value);
    //if(!value)
    //{
    ...
    if (value)
    {
        arpLatchReset.setValue(0);
        ArpRetrigger.setBypassed(true);
        ArpRangeLow.setValue(0);
        ArpRangeLow.changed();
        ArpRangeHigh.setValue(127);
        ArpRangeHigh.changed();
        // turn off midi thru...
        IgnoreInput.setValue(0);
        IgnoreInput.changed();
    }
...
inline function onarpLatchResetControl(component, value)
{
	...
	Arpeggiator1.setAttribute(Arpeggiator1.Hold, value);
    ArpRetrigger.setBypassed(1 - value);
    arpHold.setValue(0);
    //if(!value)
    //{
        Arpeggiator1.setAttribute(Arpeggiator1.ResetButton,1);
...
inline function onArpRangeLowControl(component, value)
{
	//
	if (arpHold.getValue() == 1 || arpLatchReset.getValue() == 1)
    {
        ArpRangeLow.setValue(0);
        ArpRangeHigh.setValue(127);
        arpRanger.setAttribute(0,0);
        arpRanger.setAttribute(1,127);
    }
	if (ArpRangeHigh.getValue() < value)
    {
	    ArpRangeHigh.setValue(value);
	    ArpRangeHigh.changed();
    }
    arpRanger.setAttribute(0,value);
    
};

Content.getComponent("ArpRangeLow").setControlCallback(onArpRangeLowControl);


inline function onArpRangeHighControl(component, value)
{
	//
	if (arpHold.getValue() == 1 || arpLatchReset.getValue() == 1)
    {
	...
	if (ArpRangeLow.getValue() > value)
    {
	    ArpRangeLow.setValue(value);
	    ArpRangeLow.changed();
    }
    arpRanger.setAttribute(1,value);
};

Content.getComponent("ArpRangeHigh").setControlCallback(onArpRangeHighControl);
...
PresetPanel.setTimerCallback(function()
{
    ...
    if ( (ArpUp.getValue() + ArpDown.getValue() + ArpUpDown.getValue() + ArpDownUp.getValue() + ArpRandom.getValue() + ArpChords.getValue() ) == 0)
    {
        ArpUp.setValue(1);
        ArpUp.changed();
    }
...
inline function onAttackControl(component, value)
{
	...
	for(i=0; i<NUM_VOICES;i++)
    {
        ...
        TheEnvelopes[i].setAttribute(TheEnvelopes[i].AttackCurve, 0.83);
    };
    EnvPanelAttack.setValue(value);
};
...
inline function onReleaseControl(component, value)
{
    ...
    EnvPanelRelease.setValue(value);
};
...
inline function onGridSoundsButtonControl(component, value)
{
	...
	if(value)
    {
	    Engine.openWebsite("www.nightfoxaudio.com");
	    component.setValue(0);
    }
};
